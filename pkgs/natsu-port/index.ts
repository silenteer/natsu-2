...
function connectWS<A extends NatsChannel<string, unknown, unknown>>(
  ...
  let websocketClient: WebsocketClient | undefined = new WebsocketClient(options.serverURL.toString());

  websocketClient.onerror = (event) => console.error(event);

  const sendSub = async (subject: string, headers: {}) => {
  ...
  websocketClient.onmessage = (event) => {
    try {
      ...
      websocketClient?.onerror?.(error as any);
    }
  };
  ...
  const subscribe: Subscribe<A> = async (subject, onHandle) => {
    ...
    if (websocketClient && await websocketClient.getReadyState()) {
      //OPEN
      await sendSub(subject, { ...options.headers });
