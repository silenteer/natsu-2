...
export function validateHttpRequest(request: FastifyRequest) {
...
export function validateWSRequest(request: NatsPortWSRequest) {
	let result: {
		code: 'OK' | 400;
	};
...
export async function getNamespace(params: {
	...
	if (shouldSetNamespace) {
		...
		const natsResponse = decode(message.data);
		const namespace =
...
export async function sendNatsAuthRequest(request: FastifyRequest) {
...
export async function sendNatsRequest(params: {
...
export function sendWSResponse(params: {
...
export function return400(reply: FastifyReply) {
	reply.statusCode = 400;
	reply.send();
}

export function return500(reply: FastifyReply) {
